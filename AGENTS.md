...
- The marketing system gathers a qualified audience by repeatedly delivering standalone value in the product's delegated decision space defined in `context/product.md`. It selects decision-relevant audience needs and knowledge, synthesizes evidence and limitations, and expresses the result in an immediately understandable or usable form. Research volume, technical implementation, and generic fitness information are not goals by themselves.
...
The user grants standing authorization to improve this marketing system's structure, contents, owner map, schemas, workflows, methods, jobs, dependencies, and capabilities at the end of interactive work and scheduled runs. Always assess whether a concrete operational bottleneck, ownership defect, logical conflict, reliability failure, or missing capability materially limits the business purpose. The assistant may create, merge, split, replace, move, or retire owners autonomously when the change is necessary, internally scoped, verifiable, and leaves a complete migration rather than parallel legacy paths. An assessment may correctly produce no change; do not refactor for motion or create speculative infrastructure.
...
Escalate through Telegram only when work requires a new authentication or permission, API key, paid spend, an irreversible or destructive migration, or a decision outside this marketing system's authority. A structural change by itself—including a new authoritative owner, schema, workflow, job, or top-level subsystem—does not require approval when it satisfies the four invariants and the existing product-truth, account-trust, secret-handling, publication, dirty-tree-isolation, and scheduler-specific commit boundaries. No spending cap exists yet, so any non-zero spend requires approval. When escalation is required, present one smallest viable proposal, expected benefit, cost or credential, risks, and fallback.

### Scheduled autonomous production

Recurring content-generation cron runs operate under standing user authorization and are the exception to the interactive hypothesis-confirmation, copy-approval, and publication-ready designation gates below. They improve the current marketing-funnel bottleneck through controllable content hypotheses while preserving truthful claims and the autonomous-system-improvement invariants above. The scheduler job is the sole owner of exact run times and delivery routing; do not duplicate its schedule here.
...
For each scheduled run, the assistant may autonomously read newly collected evidence, continue or close leaves, create root or child hypotheses, adopt supported rules into their proper owners, allocate one content, use accepted bounded knowledge from `db/research.sqlite`, refine unused message or copywriting versions, create new messages, and create new immutable versions when used guidance changes. It may also implement a verified structural or workflow improvement when a concrete system defect materially limits the cycle, provided the change satisfies the autonomous-system-improvement contract and is not unrelated work. Scheduled autonomy never authorizes TikTok publication.
...
When a scheduled run judges that an additional Hermes skill, plugin, package, API-backed integration, or other external capability is materially needed, it may inspect and install that capability without advance approval. Admit only the narrow missing operational capability: do not install anything that creates a parallel owner for product truth, messages, copywriting, imagery, hypotheses, content records, or publication, and do not install automatic TikTok publishing. Inspect source, trust, credentials, network calls, cost, telemetry, generated files, and side effects before installation. After any installation, report through Telegram what was installed, its source, why it was needed, and any ongoing cost, credential requirement, or material side effect. If installation cannot succeed without a new credential, permission, paid spend, destructive migration, or out-of-scope decision, request that exact user action through Telegram instead of bypassing the boundary.

The assistant autonomously selects a medium and format, then drafts, creates, validates, renders, records, and sends one publication-ready TikTok content to the configured Telegram destination. The user publishes it manually. A delivered content row remains unpublished until a TikTok URL is received; the receipt time of that URL is `published_at`. If a previously delivered content has no URL, ask for it in the next scheduled Telegram update without blocking the next content run. If the user rejects a delivered post, apply the feedback, revise or regenerate the same unpublished content and project, update its exact medium-specific copy snapshot, caption, and checksum, and redeliver it rather than creating another content identity. If credentials, unavailable product truth, paid access, destructive change, or an out-of-scope decision blocks valid completion, explain the exact blocker and required user action through Telegram instead of guessing.

After successful medium-appropriate delivery, return one compact Korean research digest after the media. It must identify the preflight research run, questions and finding IDs or terminal outcomes, sources and limitations, actions and owners, any system-integrity correction, and the resulting content-decision change; omit routine render, hash, metric, commit, and follower detail. If user action is required, return one concise Korean blocker line instead.

### Event-driven autonomous research

Research has no standalone hourly or topic-rotation cadence. It runs only from a real decision event: every content cycle performs a `content_preflight`; insertion of a new 24h, 48h, or 72h content checkpoint triggers `result_review`; and an explicit interactive request may use `manual`. A collector tick with no new checkpoint remains deterministic and starts no research agent. All triggers share `docs/research-loop.md`, `scripts/research_store.py`, and the singleton `research_runs` lease.
...
Every completed event research run returns a compact Telegram digest even when no external search was needed. The user may judge a run or finding as `useful`, `weak_evidence`, `irrelevant`, `overstated`, or `correction`; record that feedback with its non-secret Telegram evidence through `scripts/research_store.py quality-feedback`. `research_quality_feedback` owns this immutable user-quality history. Later research reads it to improve question selection, source sufficiency, scope, and admission without rewriting the original finding or agent review. A correction or removal still follows the exact withdrawal workflow and preserves provenance.

### Automated system-integrity discovery

`scripts/system_integrity.py` owns deterministic internal checks for SQLite integrity and foreign keys, schema versions, terminal research states, expired research leases, required owners, event-driven scheduler topology, prior scheduler failures, and Telegram delivery configuration. The hourly due-results wrapper and exact-minute production wrapper run it before mutating state; any failure is sent through their existing Telegram delivery route and blocks that cycle. Each event research cycle separately assesses semantic defects—missing or duplicate ownership, logical conflicts, stale workflow instructions, unreliable transitions, and materially missing capabilities—and may implement the smallest internally authorized verified correction.

This is intentionally scheduler-internal. Do not add a launchd service, daemon, heartbeat, or any other Runtime Watchdog outside Hermes Scheduler. Therefore this layer can detect failures while an existing scheduler job runs, but it does not claim to detect the Hermes scheduler process itself being stopped.
