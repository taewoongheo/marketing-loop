...
- During the current prelaunch phase, TikTok content does not mention or promote the app or its planned capabilities. It delivers standalone value to the relevant strength-training audience while improving the nearest observable and actionable upstream constraint; product exposure begins only after launch or an explicit user change to this rule. Attention, trust, and audience qualification are interpretations supported by evidence, not funnel stages.
...
During unattended work, first exhaust safe autonomous means within this system's authority, including repository and database state, compliant public sources, already approved capabilities, and any fallback that preserves a valid result. If information, access, an asset, or a user-only action that the assistant cannot obtain is materially necessary to make a valid decision or complete the work, request it through Telegram regardless of type. Ask for the smallest exact action or observation and state the decision or completion it unlocks; never request optional evidence, a choice already delegated to the assistant, or secret values in chat. A new authentication or permission, API key provisioning, paid spend, an irreversible or destructive data change, or a decision outside this marketing system's authority always requires escalation. A structural change by itself—including a new authoritative owner, schema, workflow, job, or top-level subsystem—does not require approval when it satisfies the four invariants and the existing product-truth, account-trust, secret-handling, publication, dirty-tree-isolation, and scheduler-specific commit boundaries. No spending cap exists yet, so any non-zero spend requires approval. Present one smallest viable proposal or request with its expected benefit, cost or credential purpose, risks, and fallback when applicable.

### Scheduled collection

No recurring content-production job is active. Content creation is interactive and follows the hypothesis-confirmation, copy-approval, publication-ready designation, and manual TikTok publication gates below.
