...
- Every content uses exactly one `medium`: `slideshow` or `video`. `context/production-formats.json` is the closed allowlist for new production: the assistant selects the medium and format only from its allowed entries, then uses the current hypothesis plan, project context, available evidence and assets, prior content records, and execution fit to choose among them.
- Visual and audiovisual composition is not template-driven. The assistant designs each content project from the selected format's designated references, final copy, current imagery guidance, and up to three retained same-medium same-format execution examples.
...
- `slideshow` or `video`, then one production-allowed format within that medium;
...
- adding or removing entries in the production-format allowlist;
- TikTok publication and returning the published TikTok URL.

Do not ask the user to make routine content decisions or approve production. If missing product truth, evidence, or assets prevent a valid artifact, skip the slot and report the exact blocker. Do not publish to TikTok, commit changes, delete data, or contact anyone without explicit instruction.

### Autonomous system improvement

The user grants standing authorization to improve this marketing system's internal contents, implementation details, methods, capabilities, structure, and operating model at the end of interactive work and scheduled runs. Always assess whether a concrete operational bottleneck, ownership defect, logical conflict, reliability failure, or missing capability materially limits the business purpose. Apply the smallest complete internally authorized remedy, verify it, and notify the user afterward. An assessment may correctly produce no change; do not refactor for motion or create speculative infrastructure.

The user delegates full responsibility for every internal means used to pursue the objective. The qualified App Store inflow objective defined in `docs/marketing-funnel.md` is the sole immutable element of this marketing system. The assistant may autonomously change internal policy, implementation, prompts, algorithms, queries, validations, the hypothesis system or its axes, funnel operating model, research or measurement lifecycle, content workflow, authoritative owner boundaries, schemas or durable state model, renderer architecture, scheduler topology, and top-level subsystems when the change is the smallest complete improvement. Existing structures are replaceable current implementations rather than permanent constraints. This delegation does not authorize changing product truth or taking external, destructive, credentialed, paid, publication, contact, or account-trust actions that otherwise require user involvement.
...
During unattended work, first exhaust safe autonomous means within this system's authority, including repository and database state, compliant public sources, already available capabilities, and any fallback that preserves a valid result. If information, access, an asset, or a user-only action that the assistant cannot obtain is materially necessary to make a valid decision or complete the work, request it through Telegram regardless of type. Ask for the smallest exact action or observation and state the decision or completion it unlocks; never request optional evidence, a choice already delegated to the assistant, or secret values in chat. A new authentication or permission, API key provisioning, paid spend, an irreversible or destructive data change, a decision outside this marketing system's authority, or an unresolved account-trust or consistency risk always requires escalation. Internal structural and operating-model changes do not require prior approval; apply and verify them under the four invariants, then notify the user. No spending cap exists yet, so any non-zero spend requires approval. Present one smallest viable request with its expected benefit, cost or credential purpose, risks, and fallback when applicable.

### Scheduled production and collection

One recurring reasoning-driven content-production job completes one content workflow two hours before each fixed daily publication slot: 23:00 KST for the next day's 01:00 slot, then 09:00, 12:00, and 20:00 KST for the same day's 11:00, 14:00, and 22:00 slots. The current operating target is therefore four TikTok contents per day. Each scheduled run performs the required live diagnosis and content preflight, autonomously applies the hypothesis action and one-content allocation, creates and verifies the native project and final media, records the publication-ready content, and delivers the exact media, title, caption, and publisher-only instructions to Telegram. The user is not asked to approve direction, copy, or visual readiness.

Scheduled production never publishes to TikTok. If a run starts outside its intended lead-time window or cannot complete a valid publication-ready artifact in time, skip that slot rather than producing a catch-up post at an unplanned time. Do not move unused slots to another day or increase another slot's allocation automatically.
...
Each run frames the governing question as what would most improve the current qualified-audience or content decision. It reads prior quality feedback and selects no more than three independent bounded questions, selecting zero when accepted evidence is sufficient. For every selected question, investigate actively across appropriate query formulations, source classes, and relevant domains until the minimum evidence is met, credible sources are saturated, or a material limitation is established. A result-review run treats one checkpoint as diagnostic evidence, not causal proof, and distinguishes distribution noise, measurement gaps, message, copywriting, topic, and execution conditions before proposing a next decision. Research may supply a hypothesis candidate but cannot mutate hypothesis lineage outside its separate contract.
...
`scripts/system_integrity.py` owns deterministic internal checks for SQLite integrity, lifecycle states, leases, required owners, the production-format allowlist and pending content eligibility, scheduler topology, prior job outcomes, and Telegram delivery. It also reports operational-health warnings for hypothesis stagnation, result checkpoints without review, stale TikTok Studio requests, repeated low-yield research outcomes, and unexplained concentration in finding owners or source classes. A warning prompts live diagnosis; it is not a quota and does not block the cycle by itself. Structural failures block mutation and use the existing Telegram delivery route.
...
5. Read `context/production-formats.json`, then validate each allowed `(medium, format_id)` against its renderer namespace, copywriting owner, and designated references. The file is a closed allowlist: format folders not listed there are unavailable for production even when technically renderable. If no allowed entry is valid for the run, skip the content rather than substituting a non-allowed format.
6. Select exactly one medium and format for each allocated content only from the valid allowed entries, using the applied hypothesis plan, message and copy needs, available references and production assets, execution feasibility, recent medium/format diversity, and relevant DB observations. Performance may inform operational distribution but never turns medium or format into a hypothesis axis.
...
9. If missing product truth, evidence, or assets would make the content invalid, stop that workflow and report the exact blocker rather than asking the user to choose a direction or approve a substitute.
10. Prepare the current run's funnel diagnosis, hypothesis action, and allocation decision, then apply it autonomously before creating the selected medium's native project from admitted evidence.

Before opening prior Project JSON, select at most three same-medium same-format projects solely as execution examples. Select from metadata, file validity, composition relevance to the current final copy, recency, and cross-project execution diversity. Do not rank or select them by hypothesis lineage, message/copywriting identity, publication status, or performance. Three is a maximum, not a target; do not add a weaker candidate merely to fill the count.
...
2. The assistant reads `docs/marketing-funnel.md`, product truth, relevant accepted Research DB expertise, audience language and marketing methods through their policy entrypoints, current account-wide imagery guidance, `docs/hypothesis-loop.md`, message definitions, the production-format allowlist and corresponding namespaces, and relevant DB lineage and results. It then performs one `content_preflight` research run through `docs/research-loop.md`, selecting zero questions when accepted evidence is sufficient and up to three only when the answer could materially change the content decision.
...
4. Diagnose the current bottleneck, then independently decide whether to continue an active leaf, create one or more root or child hypotheses that change a controllable output to address it, close a leaf, or adopt a supported hypothesis, and allocate the requested `n` contents.
5. Apply the hypothesis actions and allocation autonomously. For a scheduled run, allocate exactly one content to the named target publication slot. A hypothesis may generate several contents; one parent may generate any number of child hypotheses.
6. Independently select the problem, situation, message, content direction, product exposure, production-allowed medium and format, copy approach, and full medium-appropriate composition inside the applied hypothesis plan. Before project or content-record creation, require `python3 scripts/system_integrity.py --selected-medium <medium> --selected-format-id <format-id>` to return `ok: true` for the selected pair.
7. If a missing fact, evidence item, or required asset blocks truthful, audience-appropriate, or valid production, stop and report the exact blocker rather than requesting a content decision.
8. Read the selected format's copywriting version, format-specific `imagery.md` when present, all designated references, and only the selected at-most-three same-medium same-format Project examples. Draft, evaluate, and improve the copy internally using relevant accepted `expertise_entries`, the selected message, product truth, the selected copywriting version, and reviewed preflight findings. If a needed claim remains unresolved, do not add an ad-hoc tip directly to copy.
9. Finalize idiomatic U.S.-English copy and caption autonomously. For a slideshow, preserve the exact ordered text layers for every slide. For a video, preserve the exact ordered on-screen text and spoken text separately, including an empty channel when unused.
10. Create the native editable project under `renderer/<medium>/formats/<format-id>/contents/` with the same `formatId`. Derive recurring format characteristics separately from one-off execution and design the complete project without copying exact coordinates or timeline values. When managed imagery is selected, read the final project copy, current account-wide `context/imagery.md`, and selected format's `imagery.md` when present, choose content-specific geometry, then generate or select each eligible image accordingly. Project and asset creation do not create a content DB record.
11. Validate the native project, render the complete output, inspect every relevant slide or sampled video segment, and repair the project until the artifact is publication-ready. The assistant, not the user, makes the publication-ready designation.
12. Record the content under the hypothesis that generated it, together with `medium`, format identity, message identity/version, copywriting version, the exact medium-specific copy snapshot, caption, and final project path and hash. Slideshow snapshots contain one non-empty text array per slide. Video snapshots contain ordered `on_screen_text` and `spoken_text` arrays; either or both may be empty.
13. Deliver the exact final media without waiting for another request. For a slideshow, extract its PNG slides and send them in exact order as one Telegram document media group so Telegram preserves the original bytes; do not send the ZIP, separate documents, or compressed photos unless explicitly requested. For a video, send the exact rendered video file. Attach the final post title followed by the final caption—description plus tags—to the slideshow group's first document or the video message. When the delivered content contains any AI-generated imagery, video, or audio, append a clearly separated publisher-only note—outside the audience-facing title and caption—to turn TikTok's `AI-generated content` setting on. Ask only for the TikTok URL after the user publishes manually.
14. Resolve the supplied TikTok URL to one publication-ready content before writing anything. A URL-only message may be linked automatically only when the conversation and the set of publication-ready contents without URLs identify exactly one clear candidate. If no candidate or several plausible candidates remain, ask which content was published and do not record the URL or `published_at` until the user resolves it.
15. Once the content identity is certain, record the URL and `published_at` together; the shared collector then records results at 24, 48, and 72 hours. Keep observations separate from interpretations.
...
- Closed allowlist of medium/format pairs eligible for new publication-ready production and delivery: `context/production-formats.json`
...
- A renderer receives an already selected medium, format, final copy, and content-specific composition. It only reads, edits, validates, stores, and renders native projects; it never selects strategy, medium, format, or content direction.
- A renderer format namespace may exist without production permission. Non-allowed formats may be inspected, edited, technically validated, and test-rendered, but they must not generate a new publication-ready content record or delivered post. Published historical records remain valid provenance after later allowlist removal.
...
- Apply operational adoption as part of the autonomous hypothesis decision and update the one proper final owner directly. Keep the underlying observations, interpretations, and lineage in SQLite; do not duplicate them in a learning inbox.
