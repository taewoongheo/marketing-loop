...
- The `slides-marketing` JSON implements the fixed visual structure; `formats/<format-id>/guide.md` owns the copywriting grammar and adaptation reasoning tied to that structure.
- `/Users/taewoongheo/Projects/slides-marketing` is the renderer. It owns templates, image assets, editable project JSON, renderer code, and rendered images.
- Marketing context or records that still exist in `slides-marketing` are legacy copies and are not sources of truth.
...
4. Inspect the live templates in `slides-marketing/templates/`; never rely on duplicated layout notes.
...
3. The assistant independently selects the problem, situation, message, format-copy approach, product exposure, experiment boundary, and `slides-marketing` template.
...
8. When the user approves the final copy, create an editable project JSON with the already-selected template in `slides-marketing`; this does not create a content DB record.
9. The user fine-tunes the visual project and provides its path inside `slides-marketing` as the publication-ready final.
...
- Layout, slide count, typography, geometry, editable layers, project JSON, and rendered media: `slides-marketing`
