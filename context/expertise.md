...
This file is the project-wide, accumulating domain-knowledge context used to create accurate and useful LIFT CODE content. It develops the working knowledge expected of a strength-training expert without tying that knowledge to one format, message, or post.

Read this file for relevant strength-training facts, mechanisms, distinctions, practical applications, evidence, and limitations before creating content in any format.

## Ownership boundary

This file owns:

- strength-training domain knowledge that can inform content across formats and platforms;
- the provenance and evidence status of that knowledge;
- practical meaning, scope conditions, exceptions, and safe content-use limits;
- corrections to domain claims when stronger evidence becomes available.
...
A domain fact belongs here even when it is first discovered while producing one format. A format may decide how to express relevant expertise, but it must not become a second owner of the underlying knowledge.
...
- Domain advice introduced in copy must be traceable to an entry here. Product facts and audience language remain traceable to their separate owners.
- Absence from this file is not evidence that a claim is false; it means the project has not yet admitted that claim as reusable expertise.
- If valid content requires missing domain knowledge, research and add the bounded entry before drafting rather than importing an ad-hoc tip directly into copy.
...
Use one section per reusable knowledge unit.
...
Keep entries self-contained enough to use without reopening an entire research session, but do not copy full papers, long raw excerpts, or audience-language collections into this file.
