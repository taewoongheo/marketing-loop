...
Message and copywriting versions identify generation-affecting hypothesis states, not every file edit. The current version may be refined until a content row first references it. After that first use, a durable change to the corresponding behavior creates the next version. A version increase alone never creates a hypothesis node: every content row records the exact message and copywriting versions used, and multiple contents generated by the same hypothesis may reference different execution versions while testing that hypothesis. Create a child only after reviewing eligible performance evidence and confirming a distinct claim on the existing `message` or `copywriting` axis. Imagery is not a hypothesis axis; user-directed improvements update unversioned `context/imagery.md` immediately. Content-specific edits remain in the final renderer project.
