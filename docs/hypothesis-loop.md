...
utility at the current relevant-interest bottleneck; saves remain a proxy and
do not directly measure profile or App Store intent.
```
...
- the full statement and `change_axis` of every proposed new hypothesis, including its expected funnel response and the reason that response should relieve the bottleneck;
...
Owns timestamped follower-count observations used to diagnose the current TikTok link-access constraint. These rows do not belong to a content or hypothesis and never become direct evidence for a message or copywriting change, audience quality, or App Store inflow.
