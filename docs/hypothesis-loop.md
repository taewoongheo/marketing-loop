...
Before proposing either a root or child, identify why the changed idea should feel personally relevant to the target user. An operational mechanism such as repeated setup, an earlier reveal, or a different hook position is not persuasive merely because it exists. The hypothesis must connect that mechanism to a recognizable user stake—such as delayed starting, lost study time, repeated effort, frustration, or another supported consequence—and explain why that stake should produce the expected response. Do not invent financial, academic, cognitive, or opportunity losses that product or audience evidence does not support.
...
Message, copywriting, and imagery versions identify generation-affecting states, not every file edit. The current version may be refined until a content row first references it. After that first use, a durable change to the corresponding behavior creates the next version. A version increase alone never creates a hypothesis node: every content row records the exact message, copywriting, and nullable imagery versions used, and multiple contents generated by the same hypothesis may reference different execution versions while testing that hypothesis. Create a child only after reviewing eligible performance evidence and confirming a distinct claim on the existing `message` or `copywriting` axis. Imagery remains a fixed-format execution condition rather than a third hypothesis axis. Content-specific edits remain in the final renderer project and do not create durable-owner versions.
...
- `imagery_version`: exact format-coupled visual expression and image-generation guidance used, or `NULL` for historical content made without a managed imagery version;
...
Before recording a supplied URL, resolve it to exactly one publication-ready content. If a URL-only message does not identify one clear candidate, ask the user which content it belongs to and leave both fields unchanged. When first recording `tiktok_url`, write `published_at` in the same database operation. A database trigger prevents later changes to a non-null `published_at`, so correcting the URL preserves the original publication timestamp.
